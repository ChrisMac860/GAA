...
export default function ResultsSearch({ results, initialQuery }: { results: Fixture[]; initialQuery: string }) {
  ...
  const filtered = useMemo(() => {
    ...
    return index.filter(({ idx }) => words.every((w) => idx.includes(w))).map((x) => x.f);
  }, [index, debounced]);
  ...
  return (
    <div className="space-y-3">
      <label className="sr-only" htmlFor="qr">Search</label>
      <input
        id="qr"
        type="search"
        inputMode="search"
        placeholder="Search results (Irish or English)"
                ...
                {items.map((f) => (
                  <li key={f.id} role="listitem">
                    <article className="rounded-2xl border border-gray-200 p-4 hover:shadow-sm">
                      ...
                      {f.status === 'FT' ? (
                        <div className="mt-1 text-sm font-semibold text-emerald-700">{f.score}</div>
                      ) : f.status === 'PP' ? (
                        <div className="mt-1 text-sm font-semibold text-amber-700">PP</div>
                      ) : null}
                    </article>
                  </li>
                ))}
